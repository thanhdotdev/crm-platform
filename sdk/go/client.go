...
// TrackBatch sends multiple events in a single request.
func (c *Client) TrackBatch(ctx context.Context, events []Event) error {
	...
	for i, e := range events {
		payloads[i] = eventPayload{
			...
			ExternalUserID: e.UserID(),
			Data:           e,
			Timestamp:      time.Now(),
		}
	}

	body, err := json.Marshal(map[string]interface{}{"events": payloads})
	if err != nil {
		return fmt.Errorf("crmsdk: failed to marshal batch: %w", err)
	}

	req, err := http.NewRequestWithContext(ctx, http.MethodPost,
		c.endpoint+"/api/v1/ingest/events/batch", bytes.NewReader(body))
	if err != nil {
		return fmt.Errorf("crmsdk: failed to create request: %w", err)
	}
	...
	resp, err := c.http.Do(req)
	if err != nil {
		return fmt.Errorf("crmsdk: batch request failed: %w", err)
	}
	defer resp.Body.Close()
